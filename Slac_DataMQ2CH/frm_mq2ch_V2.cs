...
namespace Slac_DataMQ2CH
{
    ...
    public partial class frm_mq2ch_V2 : Form
    {
        ...
        {
            this.Text = "AD振动数据分析系统-解析:" + MQexchange + "_Line" + lineID;
            this.notifyIcon1.Text = "Slac_DataMQ2File: " + MQexchange + "_Line" + lineID;

            DataProcess dataProcess = new DataProcess();
            dataProcess.EventReceiveCount += ShowLog;

            dataProcess.Run_ReciveInfo();
        ...
        public void ShowLog(string msg, int count)
        {
            if (CheckBox_IsShowLog.Checked)
            {
                if (this.InvokeRequired)
                {
                    TextBox_Log3.BeginInvoke(new Action(() =>
                    {
                        if (TextBox_Log3.Lines.Length > 500)
                        {
                            TextBox_Log3.Clear(); // 清空 TextBox
                        }
                        TextBox_Log3.AppendText($"{msg}：消费处理数据包 {count}个 @ {DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}" + Environment.NewLine);
                        TextBox_Log3.ScrollToCaret(); // 滚动到最新一行
                    }));
                    ...
                    try
                    {
                        ...
                        if (!isPause_save2db)
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                {
                                    RichText_InputText(ex.ToString());
                                }
                            ...
                            {
                               // errorLog.AppendLine($"{DateTime.Now}@:数据包标记大小与实际不匹配！标记大小：{packet_head_list[9]}-实际大小：{bufferMQ.Length}");
                                if (checkBox5.Checked)
                                {
                                    RichText_InputText($"数据包标记大小与实际不匹配！通讯版本{ packet_head_list[0]}；机器{ packet_head_list[2]} ；传输包大小{ packet_head_list[9]}；实际大小：{ bufferMQ.Length}");
                            ...
                            try
                            {
                                ...
                                for (int i = 0; i < int_datalist.Count(); i++)
                                {
                                    ...
                                    {
                                        int[] datahead = dataVer == "1" ? SlacPlcDataV2.data_head(int_datalist[i]) : SlacPlcDataV0.data_head(int_datalist[i]);  //依次是：包ID、信息ID、时、分、秒
                                        DateTime time_data = time_Plc_Receive.Date.AddHours(datahead[2]).AddMinutes(datahead[3]).AddSeconds(datahead[4]);
                                        ...
                                        int local_milli_second = datahead_event2[2]; //本地时间毫秒
                                        ...
                                        nowdevice = packet_head_list[2].ToString();
                                        nowmsg = msg_id.ToString();
                                        int data_value = int_datalist[i + 2];
                                        int enDatavalue = dataVer == "1" ? SlacPlcDataV1.GetEnIntV1(data_value, packet_head_list[2], msg_id) : SlacPlcDataV0.GetEnIntV1(data_value, packet_head_list[2], msg_id);
                                        //SqlString.Append("('" + time_Plc_Receive.AddMilliseconds(-plctime_delta).ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss.fff") + "','" + packet_head_list[2].ToString() + "','" + msg_id.ToString() + "','" + enDatavalue.ToString() + "'),");
                                        //2024-06-24 cwz 改为保存PLC原始时间，与接收时间不再关联
                            ...
                            {
                                errorLog.AppendLine($"{DateTime.Now}@ RabbitMq数据解析转换异常:{ex.ToString()}。通讯版本{ packet_head_list[0]}；机器{ packet_head_list[2]} ；传输包大小{ packet_head_list[9]}；实际大小：{ bufferMQ.Length}");
                                if (checkBox5.Checked)
                                {
                                    RichText_InputText($"RabbitMq数据解析转换异常。通讯版本{ packet_head_list[0]}；机器{ packet_head_list[2]} ；传输包大小{ packet_head_list[9]}；实际大小：{ bufferMQ.Length}");

                                    string exepath2 = System.Reflection.Assembly.GetExecutingAssembly().Location;
                                    string FilePath2 = System.IO.Path.GetDirectoryName(exepath2);
                                    string sPath2 = FilePath2 + @"\" + DateTime.Now.ToString("yyyyMMdd");
                                    if (!Directory.Exists(sPath2))
                                    {
                                        Directory.CreateDirectory(sPath2);
                                    }
                                    string fileName2 = sPath2 + @"\" + lineID + "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss_") + DateTime.Now.ToString() + ".sbin";
                                    bool saveRet2 = FileHelper.ByteToFile(bufferMQ, fileName2);
                                }
                                    throw ex;
                            }
